...
현재는 전통적인 방식인 일반 조합요소를 이용한 한글 조합형 폰트 제작과 스마트컴포넌트를 이용한 한글 조합형 폰트 제작 기능을 포함하고 있습니다.
(스마트컴포넌트 조합 기능은 구현되어 있음에도 불구하고 샘플 예제 폰트를 확보하지 못해 이미지를 추가하지 못하였으나 확보하는대로 이미지를 추가하겠습니다.)

## 사용 금액 (5년)
**개인** : 15만원 ($150 USD) / **회사** : 20만원 ($200 USD) / **구매문의** : nanife@live.com  
...
**같은 스마트컴포넌트 값** : 동일한 스마트컴포넌트 값을 사용하는 글립들 보기
같은 스마트컴포넌트를 사용하는 글립들 중, 동일한 스마트컴포넌트 값들을 사용하는 글립들만 모아 보기  
* 옵션이 설정된 상태에서는 현재 선택된 스마트컴포넌트의 값을 조절할 경우, 동일한 스마트컴포넌트 값을 갖고 있는 글립들의 스마트컴포넌트 값이 동기화

![normal view3](./images/normal_view3.png)  
<같은 스마트컴포넌트 값을 갖는 글립들만 모아 보기>
...
* 조합요소의 이름이 선택한 자소타입(초성, 중성, 종성)에 따라 자동으로 자소 이름과 함께 비어 있는 가장 작은 인덱스를 찾아 자동으로 붙음 : 예) 초성 ㅁ 선택 시: 'mieumCho-ko' + 자동 계산된 가장 작은 인덱스 값 '.002'  
* 선택된 자소타입을 다른 자소타입의 어느 자소 그룹에 보낼지를 선택하는 방식으로, 선택된 자소 부분은 선택이 비활성화(즉, 초성 조합요소로 만들려면 초성 선택하고 해당 초성을 사용할 중성과 종성 그룹을 선택)    
* 조합요소의 글립노트는 조합요소의 용도를 위해 본인이 원하는 값을 입력하면 됨(이미 같은 이름이 있으면 '.001', '.002', '.003' 등은 자동으로 계산되어 저장됨)  
...
### 4) 스마트 컴포넌트를 보내기(편집창)
조합요소로 사용된 스마트 컴포넌트를 선택한 후 조합요소 만들기를 하면, 스마트 컴포넌트와 적용된 스마트 컴포넌트의 값들이 그대로 다른 글자들에 붙여넣기 됨



## 3. 조합 보내기
이미 조합이 완성된 글립들을 선택해 다른 글자들에 조합데이터를 붙여넣는 기능(단, 어떤 자소타입에 대해서 나머지 타입의 조합데이터를 붙여넣을지 여부 결정 :  초성 선택 시, 초성에 중성과 종성을 보냄)

![send composite0](./images/send_composite0.png)  
<선택된 글립들의 조합요소를 다른 글자들에 보내기>  

### 1) 원본 글립 선택
조합데이터를 보내기 위해서는 대상 자소타입에 대해서는 1개의 자소만 선택해야 함  
예) 초성을 대체해서 붙여넣기 위해서는 선택한 글립들에 대한 초성이 모두 동일해야 함(즉, '감', '남'을 선택하면 중성과 종성이 2개 글자에 모두 있으니 어떤 글자에서 'ㅏ' + 'ㅁ'을 가져갈지 선택할 수 없음)

### 2) 자소타입 선택
조합 데이터를 보내기 위해서는 어떤 자소타입에(해당 자소타입을 제외한 자소들을) 보낼지 선택해야 함(이 경우, 선택된 자소타입을 제외한 나머지는 비활성화)  
...
### 4) 조합 작업 방식  
**a. <조합요소 만들기> 기능을 이용해 조합을 채워 나가는 방식** : ㅁ 계열의 기본 모듈 계열의 글자들을 먼저 그린 후, 각 초, 중, 종 자소를 조합요소로 만들면서 다른 글자들에 보낸다. 이후, 조합요소가 부족한 글립들을 찾아서 부족한 요소를 그리고, 다시 다른 글자들에 조합요소로 보내는 방식을 반복하는 방식  
...
* 대표 글자에 적용된 예를 보고 선택하면 나머지 글립들에도 적용된 모습을 볼 수 있음
...
**스마트 컴포넌트 교체** : 스마트 컴포넌트는 컴포넌트 자체가 다른 경우와 컴포넌트는 같으나 스마트 컴포넌트 값이 다른 경우를 모두 선택해 교체 가능(즉, 동일한 컴포넌트에서 값만 다르게 교체하기도 가능하고, 다른 컴포넌트의 다른 값들 중 선택해 교체하기도 가능)  
...
**스마트 컴포넌트 추가** : 스마트 컴포넌트는 컴포넌트 자체가 다른 경우와 컴포넌트는 같으나 스마트 컴포넌트 값이 다른 경우를 모두 선택해 추가 가능(즉, 동일한 컴포넌트에서 값만 다른 경우도 찾아서 추가하고, 다른 컴포넌트의 다른 값들 중 선택해 교체하기도 가능)  
...
**선택한 글립과 이름 또는 노트가 같은 글립들** : 폰트 뷰에서 선택한 글립 또는 편집창에서 선택한 조합요소의 이름 또는 노트를 이용해 글립을 선택 또는 모으는 기능
...
**선택한 이름의 조합요소들 / 선택한 이름의 요소 사용 글립들**: 선택한 글립의 이름(또는 노트)과 같은 이름을 갖는 조합요소 / 해당 조합요소를 사용하는 글립들을 찾는 기능

![others13](./images/others13.png)  
<선택한 글립이름을 갖는 조합요소들 / 해당 이름의 조합요소를 갖는 글립들>  


**자소순 조합요소**: 조합요소를 순서대로 정렬하기

![others14](./images/others14.png)  
<조합요소를 초성 ㄱ 부터 종성 ㅎ까지 정렬된 상태로 모으기>  


**조합이 덜된 한글 찾기**: 초중종 조합요소 중 하나라도 없는 한글 찾기

![others15](./images/others15.png)  
<초중종 조합요소 중 하나라도 누락된 한글 검색>  


**사용되지 않는 조합요소들**: 초중종 조합요소 중 사용이 안된 조합요소들

![others16](./images/others16.png)  
<다른 글립에서 조합요소로 사용하지 않는 조합요소들>  


**조합요소 중복 글립들**: 초중종 조합요소가 추가로 더 들어 있는 글립 찾기

![others17](./images/others17.png)  
<조합요소가 겹친 글립 예>  

**조합요소 + 패스 형태의 글립들**: 조합요소와 패스가 동시에 포함되어 있는 글립 찾기
...
**모으기** : 조건에 맞는 글립들 또는 검색된 결과를 폰트뷰에서 모아보기 : 임시 작업을 위해 모아 놓은 기능으로 원상태로 되돌리기 위해서는 다른 카테고리를 누르면 됨(주의 : 많은 글립을 모으는 경우에는 시간이 오래 걸리고 부하가 크므로, 적은 글립일 때만 사용하길 권장)

![collect](./images/etc08.png)  
<글립들을 폰트뷰에서 모은 상태>

**작업 취소** : 조합요소를 추가, 변경, 이동 등의 명령을 실행했다가 해당 명령을 한번에 취소하는 기능  
![collect](./images/etc09.png)  
<바로 전에 실행된 작업을 취소하는 기능>
...
**편집창으로 열기** : 선택한 글립들을 '편집창으로 열기'를 통해 열 때, 편집창의 내용과 상관 없이 무조건 새로운 탭으로 열지 현재 편집창의 텍스트 뒤에 추가할지 결정
   
![background](./images/etc15.png)  
<편집창으로 열기 옵션>


**조합 작업 제외** : 조합 기능들을 적용할 때 제외할 대상 글립들 설정(패스 포함 글립  
* 패스 포함 글립 : 조합요소 외에 패스가 1개라도 포함된 글립들에 대한 조합 기능 적용 여부  
* 조합 완성된 글립 : 초성, 중성, 종성 조합요소가 모두 채워진 글립들에 대한 조합 기능 적용 여부   

![background](./images/etc16.png)  
<조합 작업 제외 글립들>
